...
public class Board {
	...
	public void resetBoard() {
		...
		for(int i = 0; i < 8; i++) {
			...
			this.board[6, i] = new Pawn(Piece.PieceColor.WHITE, 6, i);
		}

		for(int i = 2; i < 6; i++) {
			for(int j = 0; j < 8; j++) {
				this.board[i, j] = new None(Piece.PieceColor.NONE, i, j);
			}
		}
